...
def get_imagenet_100_datasets(train_transform, test_transform, train_classes=range(80),
                           prop_train_labels=0.8, split_train_val=False, seed=42, args=None, eval=False):
    ...
    imagenet_training_set = ImageNetBase(root=os.path.join(args.data_path, 'train'), transform=train_transform)
    ...
    test_dataset = ImageNetBase(root=os.path.join(args.data_path, 'val'), transform=test_transform)
    ...
    if eval:
        return all_datasets, subsampled_100_classes
