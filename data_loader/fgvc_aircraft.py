...
def get_aircraft_datasets(train_transform, test_transform, train_classes=range(50), prop_train_labels=0.8,
                    split_train_val=False, seed=0, args=None, only_label=False, target_transform=None):

    np.random.seed(seed)

    whole_training_set = FGVCAircraft(root=args.data_path, transform=train_transform, split='trainval')
    ...
    test_dataset = FGVCAircraft(root=args.data_path, transform=test_transform, split='test',target_transform=target_transform)

    if only_label:
        test_dataset = subsample_classes(deepcopy(test_dataset), include_classes=train_classes)
