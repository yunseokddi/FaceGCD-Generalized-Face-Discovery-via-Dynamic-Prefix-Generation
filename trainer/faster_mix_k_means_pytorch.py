...
class K_Means:
    ...
    def __init__(self, k=3, tolerance=1e-4, max_iterations=100, init='k-means++',
                 n_init=10, args=None, random_state=None, n_jobs=None, pairwise_batch_size=None, mode=None, device=None):
        self.k = k
    ...
    def fit_once(self, X, random_state):
        ...
        if self.mode == 'test':
            centers = torch.load(os.path.join(self.args.save_dir,self.args.ssk_centres))
    ...
    def fit_mix_once(self, u_feats, l_feats, l_targets, random_state):
        ...
        l_feats_cpu = l_feats.cpu()
        u_feats_cpu = u_feats.cpu()

        cat_feats = torch.cat((l_feats_cpu, u_feats_cpu))
